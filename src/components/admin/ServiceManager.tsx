  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingService) {
        ...
        try {
          await axios.post(buildApiUrl(`services/${editingService.id}`), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl(`service/${editingService.id}`), payload, config);
        }
        toast({ title: 'Success', description: 'Service updated successfully' });
      } else {
        try {
          await axios.post(buildApiUrl('services'), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl('service'), payload, config);
        }
