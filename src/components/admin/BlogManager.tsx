  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingBlog) {
        ...
        try {
          await axios.post(buildApiUrl(`blogs/${editingBlog.id}`), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl(`blog/${editingBlog.id}`), payload, config);
        }
        toast({ title: 'Success', description: 'Blog updated successfully' });
      } else {
        try {
          await axios.post(buildApiUrl('blogs'), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl('blog'), payload, config);
        }
