  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingReview) {
        ...
        try {
          await axios.post(buildApiUrl(`reviews/${editingReview.id}`), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl(`review/${editingReview.id}`), payload, config);
        }
        toast({ title: 'Success', description: 'Review updated successfully' });
      } else {
        try {
          await axios.post(buildApiUrl('reviews'), payload, config);
        } catch (err) {
          await axios.post(buildApiUrl('review'), payload, config);
        }
